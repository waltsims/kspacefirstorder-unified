...
The live tracker of what's pending on `kspacefirstorder-unified` after the mirror consolidation work. Historical planning documents are preserved under [`arxiv/`](arxiv/) for context — refer to them when you need the *why* behind decisions already made; do not edit them.
...
2. **k-wave-python URL flip** ([k-wave-python#738](https://github.com/waltsims/k-wave-python/issues/738)) — collapse `URL_DICT` to consume from the unified release; details and version-pin cleanup tracked in the issue. Ships in k-wave-python v0.6.3 or v0.7.0.
3. **Windows CUDA DLL packaging** ([#17](https://github.com/waltsims/kspacefirstorder-unified/issues/17)) — `v1.4.x` windows-CUDA ships no runtime DLLs. Bundling recipe in the issue. Unblocks dropping the Windows v1.3.0 pin in k-wave-python.
...
The adapters merge along the **backend axis** (CUDA pair → `kspace-cuda/`, OMP triple → `kspace-openmp/`, then both link the shared core). Collapsing along the platform axis (Linux pair → "linux", etc.) is the wrong axis: within a backend the C++ source is ~95% shared across platforms (only build-glue differs), but across backends within a platform it's ~30% at best (different solvers, cuFFT vs FFTW, different memory model). The subtree path refactor below is step 1 toward this end state.

#### Strategic payoff of the nanobind path

k-wave-python ships as Python wheels with the C++ adapter baked in: 5 wheels total — `linux-cpu`, `linux-cuda`, `darwin-cpu`, `win-cpu`, `win-cuda` (no `darwin-cuda` since cuFFT is Linux/Windows only). Three pain points collapse in one move:
