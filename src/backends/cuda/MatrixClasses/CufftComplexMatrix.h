 ...
 *
 * @brief     The header file containing the class implementing various FFTs
 * using the cuFFT interface.
 *
 ...
 * @brief   Class implementing ND and 1D Real-To-Complex and Complex-To-Real
 transforms using cuFFT interface.
 * @details Class implementing a single ND (3D, 2D) and many 1D Real-To-Complex
 and Complex-To-Real transforms
 *          using FFTW interface.
 * \li If the matrix is 3D, the ND transform is 3D and the batch of 1D goes over
 the second and third dimension.
 * \li If the matrix is 2D, the ND transform is 2D and the batch of 1D goes over
...
class CufftComplexMatrix : public ComplexMatrix {
 public:
   ...
   * @details This version doesn't need any scratch place for planning. All 1D
   * transforms are done in a single batch (no transpose needed) and in
   * out-of-place manner.
   *
   ...
   * @details This version doesn't need any scratch place for planning. All 1D
   * transforms are done in a single batch. Data is transposed and padded
   * according to the cuFFT data layout before the transform. The FFT is done
   * in-place.
   *
   *          This routine can be used for both 2D and 3D simulations.
   *
   ...
   * @details This version doesn't need any scratch place for planning.  All 1D
   * transforms are done in a single batch. Data has to be transposed and padded
   * according to the cuFFT data layout before the transform. The FFT is done
   * in-place.
   *
   ...
   * @details This version doesn't need any scratch place for planning. All 1D
   * transforms are done in a single batch. The output matrix is padded and
   * transposed to be padded according to the cuFFT data layout.
   *
   *          This routine can be used for both 2D and 3D simulations.
   *
   * @param   [in] outMatrixDims - The dimension sizes of the input matrix.
   ...
   * @details This version doesn't need any scratch place for planning. All 1D
   * transforms are done in a single batch. The output matrix has to be padded
   * and transposed to be padded according to the cuFFT data layout.
   *
   *          This routine can only be used for both 3D simulations.
   *
   * @param   [in] outMatrixDims - The dimension sizes of the input matrix.
  ...
  /**
   * @brief   Compute 1D out-of-place Real-to-Complex transform in the Z
   * dimension.
   * @details Compute forward out-of-place 1DY Real-to-Complex transform. The
   * matrix is first X<->Y transposed followed by the 1D FFTs. The matrix is
   * left in the transposed format.
   *
   *          This routine can be used for both 2D and 3D simulations.
   *
   * @param   [in] inMatrix      - Input data for the forward FFT.
   ...
   * dimension.
   * @details Compute forward out-of-place 1DY Real-to-Complex transform. The
   * matrix is first X<->Z transposed followed by the 1D FFTs. The matrix is
   * left in the transposed format.
   *
  ...
  /**
   * @brief   Compute 1D out-of-place Real-to-Complex transform in the Y
   * dimension.
   * @details Compute inverse out-of-place 1DY Real-to-Complex transform. The
   * matrix is requested to be in the transposed layout. After the FFT is
   * calculated, an Y<->X transposed follows. The matrix is returned in the
   * normal layout (z, y, x)  format.
   *
   *          This routine can be used for both 2D and 3D simulations.
   *
   * @param   [out] outMatrix    - Output data for the inverse FFT.
   ...
   * @brief   Compute 1D out-of-place Real-to-Complex transform in the Y
   * dimension.
   * @details Compute forward out-of-place 1DY Real-to-Complex FFT. The matrix
   * is requested to be in the transposed layout. After the FFT is calculated,
   * an Z<->X transposed follows. The matrix is returned in the normal layout
   * (z, y, x).
   *
   *          This routine can only be used for 3D simulations.
   *
 ...
 private:
   ...
   * @throw std::runtime_error with message corresponding to the cufft error
   * code.
   */
