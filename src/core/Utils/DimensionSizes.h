 ...
 * @details Default memory alignment is oriented on new, yet unknown,
 * architectures with wider SIMD units, possible 512b.
 */
 ...
 * @details Structure containing dimension sizes. The structure contains only
 * POD, so no C++ stuff is necessary. \n
 *   \li 4D objects (Nx, Ny, Nz, Nt).
 *   \li 3D objects (Nx, Ny, Nz, 0) - Nt must be 0. If we don't sample in time
 * we make 3D datasets, otherwise 4D.
 *   \li 2D objects (Nx, Ny,  1, 0) - Nt must be 0 and Nz must be 1 since all
 * datasets are stored internally as 3D.
 */
struct DimensionSizes {
  /// Default constructor.
