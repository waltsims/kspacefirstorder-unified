 ...
 *            11 February  2020, 14:49 (revised)
 *
...
#ifdef _OPENMP
  Logger::log(Logger::LogLevel::kBasic, kOutFmtUsageThreads,
              getDefaultNumberOfThreads());
#endif

  Logger::log(Logger::LogLevel::kBasic, kOutFmtUsagePart2,
              kDefaultProgressPrintInterval, kDefaultCompressionLevel);
}  // end of printUsage
//----------------------------------------------------------------------------------------------------------------------
...
void CommandLineParameters::printComandlineParamers() {
  ...
  auto formatFileName = [](const string& fileName) -> string {
    return Logger::wordWrapString(kOutFmtInputFile + fileName,
                                  kErrFmtPathDelimiters, 15)
        .c_str();
  };  // formatFileName
  ...
  if (isCheckpointEnabled()) {
    Logger::log(Logger::LogLevel::kAdvanced,
                formatFileName(mCheckpointFileName));
  }

  Logger::log(Logger::LogLevel::kAdvanced, kOutFmtSeparator);

  if (isCheckpointEnabled() && (mCheckpointInterval > 0)) {
    Logger::log(Logger::LogLevel::kAdvanced, kOutFmtCheckpointInterval,
                mCheckpointInterval);
  }
  if (isCheckpointEnabled() && (mCheckpointTimeSteps > 0)) {
    Logger::log(Logger::LogLevel::kAdvanced, kOutFmtCheckpointTimeSteps,
                mCheckpointTimeSteps);
  }

  Logger::log(Logger::LogLevel::kAdvanced, kOutFmtCompressionLevel,
              mCompressionLevel);

  Logger::log(Logger::LogLevel::kFull, kOutFmtPrintProgressIntrerval,
              mProgressPrintInterval);

  if (mBenchmarkFlag) {
    Logger::log(Logger::LogLevel::kFull, kOutFmtBenchmarkTimeStep,
...
void CommandLineParameters::parseCommandLine(int argc, char** argv) {
  ...
  // Enum with all parameter names. When adding other switches, the value shall
  // not collide with ASCII codes of short options. Enum class can't be used
  // here due to missing conversion implicit conversion to int in C++-11.
  enum Swithces {
    kHelp = 'h',
    kInputFileName = 'i',
    kOutputFileName = 'o',
  ...
  const struct option longOpts[] = {
      ...
      {"checkpoint_timesteps", required_argument, nullptr,
       kCheckpointTimesteps},
  ...
  auto reportError = [this](const string& message) {
    ...
    Logger::errorAndTerminate(
        Logger::wordWrapString(message, " ", kErrorLineIndent));
  };  // end of reportError
  ...
  while ((opt = getopt_long(argc, argv, shortOpts, longOpts, &longIndex)) !=
    ...
    switch (opt) {
      ...
      case kInputFileName: {
        // Test if the file was correctly entered (if not, getopt could eat the
        // following parameter)
        if ((optarg != nullptr) &&
      ...
      case kOutputFileName: {
        // Test if the wile was correctly entered (if not, getopt could eat the
        // following parameter)
        if ((optarg != nullptr) &&
            ((strlen(optarg) > 0) && (optarg[0] != '-'))) {
  ...
  if (!(mStorePressureRawFlag || mStorePressureRmsFlag ||
        mStorePressureMaxFlag || mStorePressureMinFlag ||
        mStorePressureMaxAllFlag || mStorePressureMinAllFlag ||
        mStorePressureFinalAllFlag || mStoreVelocityRawFlag ||
        mStoreVelocityNonStaggeredRawFlag || mStoreVelocityRmsFlag ||
        mStoreVelocityMaxFlag || mStoreVelocityMinFlag ||
        mStoreVelocityMaxAllFlag || mStoreVelocityMinAllFlag ||
...
CommandLineParameters::CommandLineParameters()
    : mInputFileName(""),
      ...
      mBenchmarkFlag(false),
      mBenchmarkTimeStepCount(0),
      mCheckpointInterval(0),
      mCheckpointTimeSteps(0),
      mPrintVersionFlag(false),
      // output flags
      mStorePressureRawFlag(false),
      ...
      mStoreVelocityRawFlag(false),
      mStoreVelocityNonStaggeredRawFlag(false),
      mStoreVelocityRmsFlag(false),
      mStoreVelocityMaxFlag(false),
      mStoreVelocityMinFlag(false),
      mStoreVelocityMaxAllFlag(false),
      mStoreVelocityMinAllFlag(false),
      mStoreVelocityFinalAllFlag(false),
...
/**
 * If -t is not used, let's check the environmental variable OMP_NUM_THREADS
 * which may be set by the user or the PBS scheduler. If it is set, let's use
 * it, otherwise, set maximum number of processors.
 */
size_t CommandLineParameters::getDefaultNumberOfThreads() {
#ifdef _OPENMP
  // Set the variable to number of logical processors in the case the env
  // variable is not set.
