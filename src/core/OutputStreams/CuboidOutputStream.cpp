 ...
 * quantities based on the cuboid sensor mask into the output HDF5 file.
 *
...
/**
 * Constructor - links the HDF5 dataset, SourceMatrix, SensorMask and reduction
 * operator together.
 */
CuboidOutputStream::CuboidOutputStream(Hdf5File& file,
    ...
    : BaseOutputStream(file, groupName, sourceMatrix, reduceOp, bufferToReuse),
      ...
      mCuboidsInfo(),
      mGroup(H5I_BADID),
      mSampledTimeStep(0) {}  // end of CuboidOutputStream
//----------------------------------------------------------------------------------------------------------------------
...
void CuboidOutputStream::reopen() {
  ...
  mSampledTimeStep = 0;
  if (mReduceOp == ReduceOperator::kNone)  // Set correct sampled time step for
                                           // raw data series
  {
    mSampledTimeStep =
  ...
  for (size_t cuboidIdx = 0; cuboidIdx < nCuboids; cuboidIdx++) {
    ...
    if (params.getTimeIndex() > params.getSamplingStartTimeIndex()) {
      if (mReduceOp != ReduceOperator::kNone) {  // Reload data
        ...
        mFile.readCompleteDataset(mGroup, datasetName, cuboidSize,
...
void CuboidOutputStream::sample() {
  switch (mReduceOp) {
    case ReduceOperator::kNone: {
      // We use here direct HDF5 offload using MEMSPACE - seems to be faster for
      // bigger datasets
      ...
      for (size_t cuboidIdx = 0; cuboidIdx < mCuboidsInfo.size(); cuboidIdx++) {
        cuboidSize = mSensorMask.getBottomRightCorner(cuboidIdx) -
                     mSensorMask.getTopLeftCorner(cuboidIdx);
        cuboidSize.nt = 1;

        mFile.writeCuboidToHyperSlab(
            mCuboidsInfo[cuboidIdx].cuboidIdx, datasetPosition,
            mSensorMask.getTopLeftCorner(
                cuboidIdx),  // Position in the SourceMatrix
            cuboidSize, mSourceMatrix.getDimensionSizes(),
      ...
      // At the time being, the using manual data lining up seems to be slower,
      // and is not used. sampleAggregated<ReduceOperator::kNone>();
    }  // case kNone
...
hid_t CuboidOutputStream::createCuboidDataset(const size_t cuboidIdx) {
  ...
  const size_t nSampledTimeSteps =
      (mReduceOp == ReduceOperator::kNone)
          ? params.getNt() - params.getSamplingStartTimeIndex()
          : 0;  // will be a 3D dataset

  // Set cuboid dimensions (subtract two corners (add 1) and use the appropriate
  // component)
  DimensionSizes cuboidSize((mSensorMask.getBottomRightCorner(cuboidIdx) -
                             mSensorMask.getTopLeftCorner(cuboidIdx))
                                .nx,
  ...
  // Set chunk size
  // If the size of the cuboid is bigger than 32 MB per time step, set the chunk
  // to approx 8MB
  ...
  hid_t dataset = mFile.createDataset(
      mGroup, datasetName, cuboidSize, cuboidChunkSize,
      Hdf5File::MatrixDataType::kFloat, params.getCompressionLevel());

  // Write dataset parameters
  mFile.writeMatrixDomainType(mGroup, datasetName,
                              Hdf5File::MatrixDomainType::kReal);
  mFile.writeMatrixDataType(mGroup, datasetName,
...
void CuboidOutputStream::sampleAggregated() {
  const size_t slabSize = mSourceMatrix.getDimensionSizes().ny *
                          mSourceMatrix.getDimensionSizes().nx;
...
// Parallelize within the cuboid - Since a typical number of cuboids is 1, then
// we have to paralelize inside
#pragma omp parallel
  for (size_t cuboidIdx = 0; cuboidIdx < mSensorMask.getDimensionSizes().ny;
       cuboidIdx++) {
    const DimensionSizes topLeftCorner =
        mSensorMask.getTopLeftCorner(cuboidIdx);
    const DimensionSizes bottomRightCorner =
        mSensorMask.getBottomRightCorner(cuboidIdx);
    ...
    for (size_t z = topLeftCorner.nz; z <= bottomRightCorner.nz; z++) {
      for (size_t y = topLeftCorner.ny; y <= bottomRightCorner.ny; y++) {
        for (size_t x = topLeftCorner.nx; x <= bottomRightCorner.nx; x++) {
          const size_t storeBufferIndex =
              cuboidInBufferStart + (z - topLeftCorner.nz) * cuboidSlabSize +
          ...
          switch (reduceOp) {
            ...
            case ReduceOperator::kRms: {
              mStoreBuffer[storeBufferIndex] +=
                  (sourceData[sourceIndex] * sourceData[sourceIndex]);
              break;
            }

            case ReduceOperator::kMax: {
              mStoreBuffer[storeBufferIndex] = std::max(
                  mStoreBuffer[storeBufferIndex], sourceData[sourceIndex]);
              break;
            }

            case ReduceOperator::kMin: {
              mStoreBuffer[storeBufferIndex] = std::min(
                  mStoreBuffer[storeBufferIndex], sourceData[sourceIndex]);
              break;
            }
          }  // switch
...
void CuboidOutputStream::flushBufferToFile() {
  ...
  for (size_t cuboidIdx = 0; cuboidIdx < mCuboidsInfo.size(); cuboidIdx++) {
    blockSize = mSensorMask.getBottomRightCorner(cuboidIdx) -
                mSensorMask.getTopLeftCorner(cuboidIdx);
    blockSize.nt = 1;

    mFile.writeHyperSlab(
        mCuboidsInfo[cuboidIdx].cuboidIdx, position, blockSize,
